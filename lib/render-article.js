...
const {resize,hasRealImage}=require('./cards');
...
module.exports=function renderArticle(slug){
 ...
 const hasImage=hasRealImage(a);
 const srcset=hasImage?[640,960,1280,1600].map(w=>`${esc(resize(a.image,w))} ${w}w`).join(', '):'';
 const caption=a.imageCaption||a.imageCredit?`${a.imageCaption?esc(a.imageCaption):''}${a.imageCaption&&a.imageCredit?' · ':''}${a.imageCredit?`Crédito: ${esc(a.imageCredit)}`:''}${a.imageSourceUrl?` · <a href="${esc(a.imageSourceUrl)}" target="_blank" rel="noopener noreferrer">Fonte da imagem</a>`:''}${a.imageLicenseUrl?` · <a href="${esc(a.imageLicenseUrl)}" target="_blank" rel="noopener noreferrer">Licença</a>`:''}`:'';
