...
module.exports=function renderArticle(slug){
 ...
 const related=content.articles.filter(x=>x.categorySlug===a.categorySlug&&x.slug!==a.slug).sort((x,y)=>y.date.localeCompare(x.date)).slice(0,4);
 ...
 const first=a.body[0]?`<h2>O que aconteceu</h2><p>${esc(a.body[0])}</p>`:'';
 const context=a.body.slice(1).length?`<h2>Contexto</h2>${a.body.slice(1).map(p=>`<p>${esc(p)}</p>`).join('')}`:'';
 ...
 const sourceLink=a.sourceUrl?`<div class="article-source-link"><a href="${esc(a.sourceUrl)}" target="_blank" rel="noopener noreferrer">Origem: ${esc(a.sourceName)} ↗</a></div>`:'';
 const articleLd={'@type':'NewsArticle','@id':`${canonical}#article`,headline:a.title,description:a.dek,datePublished:a.publishedAt||a.date,dateModified:a.modifiedAt||a.publishedAt||a.date,inLanguage:'pt-BR',isAccessibleForFree:true,articleSection:a.category,mainEntityOfPage:canonical,author:{'@type':'Organization',name:'VAELITH Editorial',url:SITE},publisher:{'@type':'NewsMediaOrganization',name:'VAELITH',url:SITE,logo:{'@type':'ImageObject',url:`${SITE}/favicon.svg`}}};
 if(hasImage)articleLd.image=[a.image];
 const ld=JSON.stringify({'@context':'https://schema.org','@graph':[articleLd,{'@type':'BreadcrumbList','@id':`${canonical}#breadcrumb`,itemListElement:[{'@type':'ListItem',position:1,name:'Início',item:SITE},{'@type':'ListItem',position:2,name:a.category,item:`${SITE}/categoria/${a.categorySlug}`},{'@type':'ListItem',position:3,name:a.title,item:canonical}]}]});
 const body=`<main class="article-shell"><div class="container"><div class="breadcrumb"><a href="/">Início</a> / <a href="/categoria/${esc(a.categorySlug)}">${esc(a.category)}</a></div><div class="article-grid"><article><header class="article-header"><div class="tag">${esc(a.category)} · ${esc(a.region)}</div><h1>${esc(a.title)}</h1><p class="article-dek">${esc(a.dek)}</p><div class="article-info"><span>${esc(a.dateDisplay)}</span></div></header>${hero}${facts}<div class="article-body">${first}${context}${takeaway}${sourceLink}<div class="ad-slot ad-slot-article"><span>Publicidade</span><strong>Espaço reservado para parceiros VAELITH</strong><a href="/midia-kit">Mídia kit →</a></div><section class="related-block"><div class="eyebrow">Continue lendo</div><h2>Relacionadas</h2><div class="related-list">${relatedList}</div></section></div></article><aside class="sidebar"><div class="sidebar-title">Mais em ${esc(a.category)}</div>${side}</aside></div></div></main>`;
 return shell({title:a.title,description:a.dek,canonical,active:a.categorySlug,body,jsonLd:ld,ogImage:hasImage?a.image:'',ogType:'article',articlePublished:a.publishedAt||a.date,articleModified:a.modifiedAt||a.publishedAt||a.date,author:'VAELITH Editorial'});
};
