...
const {card,side,image,hasRealImage}=require('./cards');
...
const adSlot=(label='Publicidade')=>`<div class="ad-slot" aria-label="${label}"><span>${label}</span><strong>Espaço reservado para parceiros VAELITH</strong></div>`;
...
module.exports=function renderHome(){
  ...
  const visualEditorial=editorial.filter(hasRealImage);
  const visualAll=all.filter(hasRealImage);

  // A home premium nunca promove uma capa editorial/fallback como se fosse fotografia.
  // A manchete considera o conteúdo validado mais recente, inclusive Pesquisa & Ciência;
  // os blocos de notícias e de editorias continuam separados por tipo de conteúdo.
  const lead=visualAll[0]||all[0];
  const sides=visualEditorial.filter(a=>a.slug!==lead?.slug).slice(0,2);
  const featured=new Set([lead&&lead.slug,...sides.map(x=>x.slug)].filter(Boolean));
  const latest=visualEditorial.filter(a=>!featured.has(a.slug)).slice(0,4);
  ...
  const researchHighlights=research.filter(hasRealImage).slice(0,3);
  const researchSection=researchHighlights.length?`<section class="section research-section"><div class="container"><div class="section-head"><div><div class="eyebrow">Pesquisa & Ciência</div><h2>Artigos científicos em destaque</h2><p>Estudos recentes com relevância prática para engenharia, construção, infraestrutura, gestão e tecnologia. As imagens são figuras originais ou oficiais validadas.</p></div><a class="section-link" href="/artigos-cientificos">Ver ranking →</a></div><div class="list-grid">${researchHighlights.map(card).join('')}</div></div></section>`:'';

  const sections=navItems.map(([slug,name],idx)=>{
    const pool=editorial.filter(a=>slug==='engenharia'?engineeringGroup.includes(a.categorySlug):a.categorySlug===slug).sort((a,b)=>b.date.localeCompare(a.date));
    if(!pool.length)return '';
    const first=pool.find(hasRealImage)||pool[0];
    const rest=[...pool.filter(a=>a.slug!==first.slug&&hasRealImage),...pool.filter(a=>a.slug!==first.slug&&!hasRealImage(a))].slice(0,2);
  ...
  const body=`<main>
    ${lead?`<section class="hero"><div class="container"><div class="hero-grid"><article class="hero-story${hasRealImage(lead)?'':' no-image'}"><a href="/noticias/${esc(lead.slug)}">${image(lead,'cover',true)}<div class="tag">${esc(lead.category)} · ${esc(lead.region)}</div><h1>${esc(lead.title)}</h1><p>${esc(lead.dek)}</p><div class="meta"><span>${esc(lead.dateDisplay)}</span></div></a></article><div class="side-stack">${sides.map(side).join('')}</div></div></div></section>`:''}
    <section class="section"><div class="container"><div class="section-head"><div><div class="eyebrow">Atualização</div><h2>Últimas notícias</h2><p>As notícias mais recentes com imagem original ou oficial validada. Os demais conteúdos continuam disponíveis no arquivo e na busca.</p></div></div><div class="daily-layout"><div class="daily-cards">${latest.map(card).join('')}</div><aside class="rank-panel"><div class="rank-title"><span>VAELITH</span><strong>Seleção editorial</strong></div>${focus}</aside></div></div></section>
  ...
  return shell({title:'VAELITH — Engenharia, Construção e Tecnologia',description:'Notícias e contexto sobre engenharia, arquitetura, construção, infraestrutura, gestão e tecnologia.',canonical:SITE,active:'home',body,jsonLd:ld,ogImage:lead&&hasRealImage(lead)?lead.image:''});
};
