...
const adSlot=(label='Publicidade')=>`<div class="ad-slot" aria-label="${label}"><span>${label}</span><strong>Espaço reservado para parceiros VAELITH</strong><a href="/midia-kit">Anuncie →</a></div>`;
...
module.exports=function renderHome(){
  ...
  const daily=sorted.filter(a=>!featured.has(a.slug)).slice(0,4);
  const dailySet=new Set(daily.map(x=>x.slug));
  const rankingItems=sorted.filter(a=>!featured.has(a.slug)&&!dailySet.has(a.slug)).slice(0,5);

  const rail=sorted.slice(0,8).map(a=>`<a class="news-rail-item" href="/noticias/${esc(a.slug)}"><span>${esc(a.category)}</span><strong>${esc(a.title)}</strong><small>${esc(a.dateDisplay)}</small></a>`).join('');
  const ranking=rankingItems.map((a,i)=>`<a class="rank-item" href="/noticias/${esc(a.slug)}"><span>${String(i+1).padStart(2,'0')}</span><div><small>${esc(a.category)} · ${esc(a.region)}</small><strong>${esc(a.title)}</strong></div></a>`).join('');
  ...
  const body=`<main>
    <section class="hero premium-hero"><div class="container"><div class="edition-line"><span class="kicker">VAELITH · EDIÇÃO DO DIA</span><span>Engenharia · Construção · Tecnologia</span></div><div class="hero-grid"><article class="hero-story"><a href="/noticias/${lead.slug}">${image(lead,'cover',true)}<div class="tag">${esc(lead.category)} · ${esc(lead.region)}</div><h1>${esc(lead.title)}</h1><p>${esc(lead.dek)}</p><div class="meta"><span>${esc(lead.dateDisplay)}</span><span>Fonte: <b>${esc(lead.sourceName)}</b></span></div></a></article><div class="side-stack">${sides.map(side).join('')}</div></div></div></section>
    ...
    <section class="section ad-section"><div class="container">${adSlot('Publicidade · Destaque')}</div></section>

    <section class="section daily-section"><div class="container"><div class="section-head"><div><div class="kicker">Seleção editorial</div><h2>Leitura essencial</h2></div><p>Os movimentos que ajudam profissionais a entender o que está mudando no ambiente construído.</p></div><div class="daily-layout"><div class="daily-cards">${daily.map(card).join('')}</div><aside class="rank-panel"><div class="rank-title"><span>VAELITH</span><strong>Em foco</strong></div>${ranking}</aside></div></div></section>
  ...
  const ld=JSON.stringify({'@context':'https://schema.org','@graph':[{'@type':'NewsMediaOrganization','@id':`${SITE}/#organization`,name:'VAELITH',url:SITE,description:'Portal de inteligência para engenharia, arquitetura, construção, infraestrutura, gestão e tecnologia.',email:'labsvaelith@gmail.com'},{'@type':'WebSite','@id':`${SITE}/#website`,url:SITE,name:'VAELITH',publisher:{'@id':`${SITE}/#organization`},potentialAction:{'@type':'SearchAction',target:`${SITE}/buscar?q={search_term_string}`,'query-input':'required name=search_term_string'}}]});
  return shell({title:'VAELITH — Engenharia, Construção e Tecnologia',description:'Notícias, análises e inteligência sobre engenharia, arquitetura, construção, infraestrutura, gestão e tecnologia.',canonical:SITE,body,jsonLd:ld,ogImage:lead.image});
};
