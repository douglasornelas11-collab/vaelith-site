...
module.exports=function renderHome(){
  ...
  const researchSection=researchHighlights.length?`<section class="section research-section"><div class="container"><div class="section-head"><div><div class="eyebrow">Pesquisa & Ciência</div><h2>Artigos científicos em destaque</h2><p>Estudos recentes com relevância prática para engenharia, construção, infraestrutura, gestão e tecnologia.</p></div><a class="section-link" href="/artigos-cientificos">Ver todos →</a></div><div class="list-grid">${researchHighlights.map(card).join('')}</div></div></section>`:'';

  const sections=navItems.map(([slug,name],idx)=>{
    const items=editorial.filter(a=>a.categorySlug===slug).sort((a,b)=>b.date.localeCompare(a.date)).slice(0,3);
