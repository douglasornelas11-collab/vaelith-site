...
module.exports=function renderHome(){
  ...
  const body=`<main>
    ...
    <section class="section"><div class="container"><div class="section-head"><div><div class="eyebrow">Agora</div><h2>Últimas notícias</h2><p>Os assuntos mais recentes do ambiente construído, com fonte identificada e contexto técnico.</p></div></div><div class="daily-layout"><div class="daily-cards">${latest.map(card).join('')}</div><aside class="rank-panel"><div class="rank-title"><span>VAELITH</span><strong>Em foco</strong></div>${ranking}</aside></div></div></section>
    ...
    <section class="section" id="agenda"><div class="container"><div class="section-head"><div><div class="eyebrow">Agenda</div><h2>Eventos do setor</h2><p>Feiras, congressos e encontros relevantes para engenharia, construção, arquitetura, gestão e tecnologia.</p></div><a class="section-link" href="/eventos">Agenda completa →</a></div><div class="event-grid"><div class="event-list">${events}</div><aside class="radar"><div class="tag">Radar VAELITH</div><h3>O que merece atenção.</h3><p>Eventos selecionados por relevância para profissionais e empresas do ambiente construído.</p><div class="radar-list"><div>Fontes oficiais</div><div>Brasil e exterior</div><div>Tecnologia e construção</div></div></aside></div></div></section>

    <section class="section alt"><div class="container"><div class="editorial-box premium-trust"><div><div class="eyebrow">Transparência</div><h2>Fonte identificada. Contexto claro. Separação entre fato e análise.</h2><p>A VAELITH produz sínteses editoriais próprias e mantém a origem das informações acessível ao leitor.</p></div><a class="trust-action" href="/politica-editorial">Política editorial →</a></div></div></section>
  </main>`;
