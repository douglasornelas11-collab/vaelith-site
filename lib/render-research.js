...
function rankedPaper(p,i,featured=false){
  return `<article class="research-ranked-item${featured?' research-ranked-featured':''}"><div class="research-rank-block"><span class="research-rank-no">${String(i+1).padStart(2,'0')}</span><span class="research-score">${p.researchScore}/100</span></div><a class="research-ranked-image" href="/noticias/${esc(p.slug)}">${image(p,'research-thumb',featured)}</a><div class="research-ranked-copy"><div class="tag">${esc(p.category)} · ${esc(p.region)}</div><h3><a href="/noticias/${esc(p.slug)}">${esc(p.title)}</a></h3><p>${esc(p.dek)}</p>${featured&&p.technicalTakeaway?`<div class="research-why"><strong>Por que importa</strong><p>${esc(p.technicalTakeaway)}</p></div>`:''}<div class="research-ranked-meta"><span>Importância: ${level(p.researchScore)}</span><span>${esc(p.dateDisplay)}</span></div></div></article>`;
}
...
module.exports=function renderResearch(){
  ...
  const day=week[0];
  const weeklyCards=week.map((p,i)=>rankedPaper(p,i,i===0)).join('');
  ...
  const body=`<main>
    ...
    <section class="section research-day-section"><div class="container"><div class="section-head"><div><div class="eyebrow">Artigo do dia</div><h2>#1 do ranking científico</h2><p>O estudo mais relevante da seleção atual, apresentado no mesmo padrão visual das demais notícias.</p></div><div class="research-score-badge"><strong>${day.researchScore}</strong><span>/100</span></div></div><div class="research-ranked-list research-ranked-day">${rankedPaper(day,0,true)}</div></div></section>
