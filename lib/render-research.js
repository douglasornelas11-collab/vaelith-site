...
module.exports=function renderResearch(){
  ...
  const day=week[0];
  const weeklyCards=week.map((p,i)=>`<article class="research-ranked-item"><div class="research-rank-block"><span class="research-rank-no">${String(i+1).padStart(2,'0')}</span><span class="research-score">${p.researchScore}/100</span></div><a class="research-ranked-image" href="/noticias/${esc(p.slug)}">${image(p,'research-thumb')}</a><div class="research-ranked-copy"><div class="tag">${esc(p.category)} · ${esc(p.region)}</div><h3><a href="/noticias/${esc(p.slug)}">${esc(p.title)}</a></h3><p>${esc(p.dek)}</p><div class="research-ranked-meta"><span>Importância: ${level(p.researchScore)}</span><span>${esc(p.dateDisplay)}</span></div></div></article>`).join('');
  const archiveHtml=archive.length?`<section class="section alt"><div class="container"><div class="section-head"><div><div class="eyebrow">Acervo científico</div><h2>Pesquisas anteriores</h2><p>Os artigos deixam o Top 7, mas permanecem disponíveis para consulta.</p></div></div><div class="research-archive-list">${archive.map(p=>`<a class="research-archive-row" href="/noticias/${esc(p.slug)}"><span>${esc(p.dateDisplay)}</span><strong>${esc(p.title)}</strong><em>${scorePaper(p)}/100</em></a>`).join('')}</div></div></section>`:'';
  ...
  const body=`<main>
    ...
    <section class="section research-day-section"><div class="container"><div class="section-head"><div><div class="eyebrow">Artigo do dia · #1 do ranking</div><h2>O estudo que merece mais atenção agora</h2><p>O ranking prioriza importância técnica. Métricas de audiência entram no cálculo quando houver dados confiáveis disponíveis.</p></div><div class="research-score-badge"><strong>${day.researchScore}</strong><span>/100</span></div></div><article class="research-day"><a class="research-day-image" href="/noticias/${esc(day.slug)}">${image(day,'cover',true)}</a><div class="research-day-copy"><div class="tag">${esc(day.category)} · ${esc(day.region)}</div><h2><a href="/noticias/${esc(day.slug)}">${esc(day.title)}</a></h2><p class="research-day-summary">${esc(day.dek)}</p>${day.technicalTakeaway?`<div class="research-why"><strong>Por que importa</strong><p>${esc(day.technicalTakeaway)}</p></div>`:''}<div class="meta"><span>${esc(day.dateDisplay)}</span><span>Importância: ${level(day.researchScore)}</span></div><a class="section-link" href="/noticias/${esc(day.slug)}">Ler análise completa →</a></div></article></div></section>

    <section class="section alt"><div class="container"><div class="section-head"><div><div class="eyebrow">Ranking semanal</div><h2>Top 7 artigos científicos</h2><p>Ordem determinada por relevância técnica, aplicabilidade, qualidade da publicação, profundidade e atualidade. Não exibimos volume de busca estimado quando não há dado confiável.</p></div></div><div class="research-ranked-list">${weeklyCards}</div></div></section>
    ${archiveHtml}
  </main>`;
