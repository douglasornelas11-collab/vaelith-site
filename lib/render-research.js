...
module.exports=function renderResearch(){
  ...
  const archiveHtml=archive.length?`<section class="section alt"><div class="container"><div class="section-head"><div><div class="eyebrow">Acervo científico</div><h2>Pesquisas anteriores</h2><p>Os artigos que saem do Top 7 continuam disponíveis para consulta.</p></div></div><div class="research-archive-list">${archive.map(p=>`<a class="research-archive-row" href="/noticias/${esc(p.slug)}"><div class="research-archive-date">${esc(p.dateDisplay)}</div><div class="research-archive-title">${esc(p.title)}</div><div class="research-archive-score">${scorePaper(p)}/100</div></a>`).join('')}</div></div></section>`:'';
  ...
  const body=`<main>
    ...
    <section class="section alt"><div class="container"><div class="section-head"><div><div class="eyebrow">Ranking semanal</div><h2>Top 7 artigos científicos</h2><p>Posições 02 a 07 da seleção atual.</p></div></div><div class="research-ranked-list">${weeklyCards}</div></div></section>
    ${archiveHtml}
  </main>`;
