...
function parse(xml,source,now=Date.now()){
  return [...xml.matchAll(/<item(?:\s[^>]*)?>([\s\S]*?)<\/item>/gi)].flatMap(([,raw])=>{
    ...
    const imageMarkup=decode(field(raw,'description')+' '+field(raw,'content:encoded')+' '+field(raw,'imagem-destaque'));
    const imageMatch=imageMarkup.match(/<img[^>]+src=["']([^"']+)["']/i)||raw.match(/<media:content[^>]+url=["']([^"']+)["']/i);
    let image='';try{const candidate=new URL(decode(imageMatch?.[1]||''));if(candidate.protocol==='https:')image=candidate.href;}catch{}
    return [{title,url:u.href,image,source:source.name,publishedAt:new Date(date).toISOString(),score:Math.min(matches.length,5)}];
  });
}
