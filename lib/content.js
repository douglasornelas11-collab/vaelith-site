// VAELITH content pipeline — imagens oficiais ou licenciadas; fallback apenas de emergência.
...
function trustedExternalImage(url='',credit='',caption=''){
  ...
  const isStock=stockHosts.some(domain=>host===domain||host.endsWith('.'+domain));
  if(isStock){
    const credited=/(unsplash|pexels|pixabay)/i.test(String(credit||''));
    const markedIllustrative=/imagem ilustrativa/i.test(String(caption||''));
    if(!credited||!markedIllustrative)return '';
  }
  return u;
}
