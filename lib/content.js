// VAELITH content pipeline — imagens oficiais/licenciadas validadas; fallback apenas de emergência.
...
function trustedExternalImage(url='',credit='',caption=''){
  const u=String(url||'').trim();
  if(!u)return '';
  if(u.startsWith('data:image/'))return '';
  if(!/^https:\/\//i.test(u))return '';
  const stock=/((^|\.)unsplash\.com|images\.unsplash\.com|(^|\.)pexels\.com|(^|\.)pixabay\.com)/i.test(u);
  if(stock){
    // Banco de imagem só é aceito quando o CMS identifica explicitamente a licença/crédito
    // e a legenda deixa claro que se trata de imagem ilustrativa.
    const rights=String(credit||'');
    const note=String(caption||'');
    if(!/(unsplash|pexels|pixabay)/i.test(rights)||!/imagem ilustrativa/i.test(note))return '';
  }
  return u;
}
