...
function editorialCover(categorySlug='',title='',slug=''){
  ...
  const safeTitle=String(title||'VAELITH').replace(/[&<>"']/g,' ').slice(0,58);
  const svg=`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1600 900"><rect width="1600" height="900" fill="#101214"/><rect x="0" y="0" width="1600" height="900" fill="${color}" opacity=".58"/><path d="M${a} 735 L${c} 150 L1460 150 L${b+520} 735 Z" fill="none" stroke="#d8ff3e" stroke-width="3" opacity=".78"/><path d="M${d} 125 H1480 M${d} 770 H1480" stroke="#ffffff" stroke-width="1" opacity=".20"/><circle cx="${c}" cy="${d+250}" r="${90+(h%80)}" fill="none" stroke="#ffffff" stroke-width="1" opacity=".18"/><text x="110" y="160" fill="#d8ff3e" font-family="Arial,Helvetica,sans-serif" font-size="34" font-weight="700" letter-spacing="7">${label}</text><text x="110" y="690" fill="#ffffff" font-family="Arial,Helvetica,sans-serif" font-size="58" font-weight="700">${safeTitle}</text><text x="110" y="760" fill="#c7ccd0" font-family="Arial,Helvetica,sans-serif" font-size="28" letter-spacing="5">VAELITH EDITORIAL</text></svg>`;
